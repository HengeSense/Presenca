...
// -------------------------------------- PROJECTS --------------------------------------- //
	...
	if (isset ($_POST['projectComplete'])) {
		...
		<?php if ($core->group == 1 /* PROJECTS */  || $core->group == 2 /* FINANCE */ || $core->level >= 10) { ?><img src="images/64-Pencil.png" class="editButton" alt="Edit"> <?php } ?>
		...
		if ($core->group == 1 || $core->level >= 10) {
			...
			if ($id == "0") {
				...
				if ($insert) {
					...
					notificationSave(array(), "<b>$core->user</b> criou o projeto <b>$name</b>.", "projects.php");	
			...
			// Update project
				...
				if ($update) {
					...
					// And now we can save the notification
					notificationSave(array(), "<b>$core->user</b> atualizou o projeto <b>$name</b>.", "projects.php");	
